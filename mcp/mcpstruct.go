...
// Defines the structure used for UI commands and their results
type UICommand struct {
	Input CommandInput `json:"input"`
	ID    string       `json:"id,omitempty"`
	Table struct {
		DisplayOptions []string `json:"display-options"`
		Rows           []struct {
			Cells []struct {
				CellType string `json:"cell-type"`
				Data     string `json:"str-datum"`
			} `json:"cell"`
		} `json:"row"`
	} `json:"table"`
...
// The command object used to request UI data
